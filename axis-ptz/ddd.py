...
while index < data.shape[0] - 1:
    ...
    # Assign last camera pan and tilt rate difference
    delta_rho_dot_c = history["delta_rho_dot_c"][-1]
    delta_tau_dot_c = history["delta_tau_dot_c"][-1]
    
    # Use last camera pan and tilt rate
    rho_dot_c = history["rho_dot_c"][-1]
    tau_dot_c = history["tau_dot_c"][-1]
    ...
    if time_c >= data["latLonTime"][index + 1]:
        ...
        # Compute slew rate differences with gain
        delta_rho_dot_c = k_rho * (rho_a - rho_c) / dt_c
        delta_tau_dot_c = k_tau * (tau_a - tau_c) / dt_c

        # Update camera pan and tilt rate
        rho_dot_c = rho_dot_a + delta_rho_dot_c
        tau_dot_c = tau_dot_a + delta_tau_dot_c
