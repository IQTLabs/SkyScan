...
def run_detection_model_tiled(dataset_name, training_name, prediction_field, tile_string, tile_overlap, iou_threshold):
    ...
    for sample in dataset.select_fields("filepath"):
        ...
        for tile_size in tile_sizes:
            ...
            for tile_location in _tiles_location_gen(img_size, tile_size, tile_overlap):
                ...
                new_im.paste(im, (0,0)) #((input_tensor_size-new_size[0])//2, (input_tensor_size-new_size[1])//2))
                ...
                for i, detectScore in enumerate(detections['detection_scores'][0]):
                    if detectScore > min_score:
                        ...
                        x1 = detections['detection_boxes'][0][i][1].numpy() * input_tensor_size #tile_width
                        y1 = detections['detection_boxes'][0][i][0].numpy() * input_tensor_size #tile_height
                        x2 = detections['detection_boxes'][0][i][3].numpy() * input_tensor_size #tile_width
                        y2 = detections['detection_boxes'][0][i][2].numpy() * input_tensor_size #tile_height
                        bbox = [x1,y1,x2,y2]
                        ...
                        repositioned_bbox = _reposition_bounding_box(scaled_bbox, tile_location)   
                        confidence = detections['detection_scores'][0][i]
